...
DetermineTailPos(struct boid *b)
{
	...
	b->tailEnd.x = b->tailHead.x - b->velocity.x * 4;
	b->tailEnd.y = b->tailHead.y - b->velocity.y * 4;
}
...
{
	// Allocate a new prey
	struct prey *p = (struct prey *)malloc(sizeof(struct prey));
...
AvoidPredator(struct boid *b)
{
	...
	if (check_within_radius(&b->centre, &MousePos, MAX_RADIUS)) { 
		return -10;
	} else {
		return 1;
	}
...
KeepDistance(struct boid *b)
{
	...
	struct vector *t = (struct vector *)malloc(sizeof(struct vector));
	...
	for (i = 0; i < MAXBOIDS; i++) {
		...
		if (check_within_radius(&b->centre, &boids[i].centre, 50)) {
